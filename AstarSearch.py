# -*- coding: utf-8 -*-
"""
Created on Thu May 25 18:30:49 2023
@author: DINA
"""
...
class Graph:
    ...
    def add_edge(self, node1, node2, cost=1, __reversed=True):
        try:
        ...
        while not found and len(fringe):
            ...
            for node in self.neighbors(current):
                ...
                if node not in visited or cost_so_far[node] > new_cost:
                    ...
                    heapq.heappush(fringe, (new_cost + self.heuristics[node], node))
...
class GUI:
    ...
    def create_nav_bar(self):
        ...
        search_button = tk.Button(nav_bar, text="Search", command=self.run_a_star_search)
    ...
    def create_edge(self, node1, node2):
        cost = float(simpledialog.askstring("Edge Cost", "Enter edge cost:"))
        self.graph.add_edge(node1.name, node2.name, cost)
        self.canvas.create_line(node1.x, node1.y, node2.x, node2.y, fill="#800080", width=2)
        self.canvas.create_text((node1.x + node2.x) / 2, (node1.y + node2.y) / 2, text=str(cost), fill="black")
    ...
    def run_a_star_search(self):
        ...
        came_from, cost = self.graph.a_star_search(start_node_name, goal_node_name)
